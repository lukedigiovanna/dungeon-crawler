...
Tilemap::Tilemap(const std::string& path, int spriteOffsetId, float scale) : scale(scale) {
    // Load the CSV file from disk
    std::ifstream file(path);

    if (!file) {
        throw std::runtime_error("Tilemap::Tilemap: file does not exist: " + path);
    }
    ...
    // Read number of commas until first newline character
    ...
    this->width = width;
    this->height = height;
    this->nChunksWidth = width / CHUNK_SIZE;
    this->nChunksHeight = height / CHUNK_SIZE;
    if (width % CHUNK_SIZE != 0) {
        nChunksWidth++;
        std::cerr << "Warning: Loading tilemap from file with unaligned chunk width";
    }
    if (height % CHUNK_SIZE != 0) {
        nChunksHeight++;
        std::cerr << "Warning: Loading tilemap from file with unaligned chunk height";
    }
    worldWidth = nChunksWidth * CHUNK_SIZE * scale;
    worldHeight = nChunksHeight * CHUNK_SIZE * scale;
    chunks = std::make_unique<Chunk[]>(nChunksWidth * nChunksHeight);
    ...
    while (ci < contents.length()) {
        ...
        if (value >= 0) {
            setTile(row, col, spriteOffsetId + value, true);
        }
        if (contents[ci] == '\n') {
            row++;
            col = 0;
